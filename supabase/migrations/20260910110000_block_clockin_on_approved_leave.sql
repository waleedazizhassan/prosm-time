-- PROSM Time - real gap found in the 14-point live-audit: clock_in_
-- prosm_time_attendance had zero references to leave_requests, so an
-- employee on approved leave today could clock in with no server-side
-- check, no override path, no audit trail. Fixed here by blocking
-- self clock-in outright when the caller has an approved leave
-- request covering today.
--
-- Deliberately self-clock-in only, not admin_clock_in_prosm_time_
-- attendance: that RPC is already this app's real, audited override
-- path for exactly this kind of exception (mandatory p_reason, a real
-- audit_logs + admin_on_behalf_actions row per call - see
-- 20260902090000_site_scoped_manager_authority.sql). A manager who
-- deliberately clocks someone in during their own leave (e.g. an
-- emergency callback) already leaves a full, reviewable trail through
-- that path - it should not be blocked a second time here.
begin;
...
begin
    ...
    -- § real gap fix, live-reported - block a fresh clock-in outright
    -- when the caller has an approved leave request covering today.
    -- Placed after the idempotency-replay check (a replay of an
    -- already-succeeded clock-in must still succeed) and before any
    -- other work, so a blocked attempt has no side effects at all.
    if exists (
