// export-payroll-deductions - PROSM Finance's payroll bridge (user-
// directed, 2026-09-14). Same auth shape as the other payroll:read
// exports. Returns each worker's total lateness-deduction minutes
// (timesheets.total_deduction_minutes) summed across every timesheet
// overlapping the window - the only monetary-adjacent deduction
// concept PROSM Time's schema has today.
...
serve(async (request: Request) => {
  ...
  try {
    ...
    const exportRateLimited = await enforceRateLimits(serviceClient, [
      { scope: "export_payroll_deductions_ip", identifier: clientIdentifier(request), limit: 60, windowSeconds: 300, blockSeconds: 900 },
      { scope: "export_payroll_deductions_key", identifier: await hashIdentifier(rawKey), limit: 60, windowSeconds: 300, blockSeconds: 900 },
    ]);
    ...
    const { data, error } = await serviceClient.rpc("list_prosm_time_payroll_deductions_export", {
    ...
    const records = (data ?? []).map((row: any) => ({
      ...
      totalDeductionMinutes: Number(row.total_deduction_minutes ?? 0),
    }));
    ...
    return errorResponse(error?.message ?? "Payroll deductions export service unavailable.", 500, "INTERNAL_ERROR");
  }
});
