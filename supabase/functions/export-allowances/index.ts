// export-allowances - PROSM Finance's payroll bridge (user-directed,
// 2026-09-14: "اظهر اللي موجود في تايم بس مفتاح API فقط"). Same auth
// shape as export-worker-rates: requires a key with payroll:read scope
// (authenticate_prosm_time_api_key_with_scope), returns only APPROVED
// daily allowance entries in the requested window.
...
serve(async (request: Request) => {
  ...
  try {
    ...
    const exportRateLimited = await enforceRateLimits(serviceClient, [
      { scope: "export_allowances_ip", identifier: clientIdentifier(request), limit: 60, windowSeconds: 300, blockSeconds: 900 },
      { scope: "export_allowances_key", identifier: await hashIdentifier(rawKey), limit: 60, windowSeconds: 300, blockSeconds: 900 },
    ]);
    ...
    const { data, error } = await serviceClient.rpc("list_prosm_time_allowances_export", {
    ...
    const records = (data ?? []).map((row: any) => ({
      ...
      expatriationAllowance: Number(row.expatriation_allowance ?? 0),
      ...
      overtimeDays: Number(row.overtime_days ?? 0),
    }));
    ...
    return errorResponse(error?.message ?? "Allowances export service unavailable.", 500, "INTERNAL_ERROR");
  }
});
